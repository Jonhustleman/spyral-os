...
export class RealityCycleService {
  ...
  async execute(
    ...
    try {
      ...
      // Build enhanced SPYRAL Reality Report (Pilot Final)
      const gapAnalysis = {
        currentState: this.extractDomain(request.goal),
        desiredState: request.goal,
        gap: `Moving from current "${this.extractDomain(request.goal)}" state toward: "${request.goal}"`,
        mainObstacles: [
          `Limited visibility into current "${this.extractDomain(request.goal)}" baseline metrics`,
          ...sop.currentReality.constraints.slice(0, 2),
          "Unknown factors that may emerge during execution",
        ],
        leveragePoints: [
          `Clear goal definition enables focused execution in ${this.extractDomain(request.goal)}`,
          `Multiple strategy options increase probability of success`,
          "Structured milestones provide early validation opportunities",
        ],
      };

      const strategyDetails = ste.strategyOptions.map((opt, si) => ({
        name: opt.name,
        explanation: opt.description,
        whyItMayWork: opt.pros.join("; "),
        steps: [
          `Step 1: Define baseline metrics and success criteria for "${opt.name}"`,
          `Step 2: Launch initial initiatives with 2-week measurement cycles`,
          `Step 3: Review progress, adapt approach based on data, and scale successful patterns`,
          ...ste.recommendedStrategy.milestones.slice(0, 2).map((m) => `Step: ${m.title} — ${m.description}`),
        ],
        resourcesRequired: [
          "Time allocation for strategic execution",
          "Measurement and tracking infrastructure",
          "Team or stakeholder alignment",
          ...(si === 2 ? ["Significant investment and organizational commitment"] : []),
        ],
        timeline: `${ste.recommendedStrategy.milestones.length} milestone phases (${ste.recommendedStrategy.milestones[0]?.timeframe ?? "phased"} through ${ste.recommendedStrategy.milestones[ste.recommendedStrategy.milestones.length - 1]?.timeframe ?? "completion"})`,
        expectedOutcome: opt.description.substring(0, 100),
        risks: sve.risks.slice(0, 2).map((r) => r.description),
        confidenceScore: `${Math.round(opt.confidence * 100)}%`,
        measurementMethod: `Track progress against ${ste.recommendedStrategy.milestones.length} milestone success criteria with ${sae.learningPathway.measurementPlan.reviewCadence}`,
      }));

      const execution72h = sae.immediateTasks.slice(0, 3);
      const execution30d = ste.recommendedStrategy.milestones.slice(0, 1).map((m) => ({
        milestone: m.title,
        action: m.description,
        criteria: m.successCriteria,
      }));
      const execution90d = ste.recommendedStrategy.milestones.slice(1).map((m) => ({
        milestone: m.title,
        action: m.description,
        criteria: m.successCriteria,
      }));

      // Confidence display per rules
      const confidenceDisplay = {
        score: Math.round(sve.confidenceScore * 100),
        reason: sve.confidenceFactors.increasesConfidence.slice(0, 2).join("; "),
        unknownFactors: sve.confidenceFactors.decreasesConfidence.filter((f) => !f.includes("strateg") && !f.includes("option") && !f.includes("milestone") && !f.includes("risk") && !f.includes("learning")),
      };
      ...
      const spyralReport = [
        ...
        ``,
        `1️⃣  CURRENT REALITY`,
        `   • Situation: Pursuing "${request.goal}" in the ${this.extractDomain(request.goal)} domain`,
        `   • Known factors: ${sop.observations.length} observations identified`,
        `   • Constraints: ${sop.currentReality.constraints.join("; ")}`,
        `   • Missing information: ${sop.currentReality.unknown.length} areas need investigation`,
        `   • Key assumptions: ${sop.assumptions.slice(0, 3).join("; ")}`,
        ``,
        `2️⃣  DESIRED REALITY`,
        `   • Target outcome: ${sop.desiredReality.substring(0, 120)}`,
        `   • Success criteria: ${ste.recommendedStrategy.milestones.map((m) => m.successCriteria).join("; ")}`,
        `   • Timeline: ${ste.recommendedStrategy.milestones[0]?.timeframe ?? "Phased"} to ${ste.recommendedStrategy.milestones[ste.recommendedStrategy.milestones.length - 1]?.timeframe ?? "completion"}`,
        ``,
        `3️⃣  GAP ANALYSIS`,
        `   • Current state: ${gapAnalysis.currentState}`,
        `   • Desired state: ${gapAnalysis.desiredState.substring(0, 80)}`,
        `   • Gap: ${gapAnalysis.gap.substring(0, 100)}`,
        `   • Main obstacles: ${gapAnalysis.mainObstacles.map((o) => `\n      - ${o}`).join("")}`,
        `   • Leverage points: ${gapAnalysis.leveragePoints.map((l) => `\n      + ${l}`).join("")}`,
        ``,
        `4️⃣  HIDDEN STRUCTURE ANALYSIS`,
        `   • Key variables controlling outcome: ${lde.hiddenPatterns.map((p) => `\n      - ${p}`).join("")}`,
        `   • Patterns detected: ${lde.deeperStructures.map((s) => `\n      - ${s}`).join("")}`,
        `   • Assumptions to validate: ${sop.assumptions.map((a) => `\n      - ${a}`).join("")}`,
        `   • Possible bottlenecks: ${sve.risks.slice(0, 3).map((r) => `\n      ⚠️ ${r.description}`).join("")}`,
        ``,
        `5️⃣  STRATEGY GENERATION`,
        ...strategyDetails.flatMap((sd, i) => [
          ``,
          `   ─── Strategy ${i + 1}: ${sd.name} ───`,
          `   • Explanation: ${sd.explanation}`,
          `   • Why it may work: ${sd.whyItMayWork}`,
          `   • Steps:`,
          ...sd.steps.map((s) => `      ${s}`),
          `   • Resources required: ${sd.resourcesRequired.join(", ")}`,
          `   • Timeline: ${sd.timeline}`,
          `   • Expected outcome: ${sd.expectedOutcome}`,
          `   • Risks: ${sd.risks.join("; ")}`,
          `   • Confidence score: ${sd.confidenceScore}`,
          `   • Measurement method: ${sd.measurementMethod}`,
        ]),
        ``,
        `6️⃣  STRATEGY RECOMMENDATION`,
        `   • Selected strategy: "${ste.recommendedStrategy.approach.substring(0, 80)}"`,
        `   • Reason selected: ${ste.recommendedStrategy.rationale.substring(0, 150)}`,
        `   • Evidence supporting choice: ${sve.validatedAssumptions.slice(0, 3).map((a) => `\n      ✓ ${a}`).join("")}`,
        `   • Uncertainty remaining: ${sve.invalidatedAssumptions.slice(0, 2).map((a) => `\n      ? ${a}`).join("")}`,
        ``,
        `7️⃣  EXECUTION PLAN`,
        `   `,
        `   ─── First 72 Hours ───`,
        ...execution72h.map((t, i) => `   ${i + 1}. ${t}`),
        `   `,
        `   ─── First 30 Days ───`,
        ...execution30d.flatMap((m) => [
        ...
        `   `,
        `   ─── Milestone Summary ───`,
        ...ste.recommendedStrategy.milestones.map((m, i) =>
          `   ${i + 1}. ${m.title} (${m.timeframe}): ${m.description.substring(0, 60)}...`
        ),
        ``,
        `8️⃣  VALIDATION LOOP`,
        `   • Prediction: Following the "${ste.recommendedStrategy.approach.substring(0, 60)}..." strategy will achieve the desired outcome within the defined timeline.`,
        `   • Measurement: Track progress against ${ste.recommendedStrategy.milestones.length} milestone success criteria. ${sae.learningPathway.measurementPlan.metrics.slice(0, 2).join("; ")}.`,
        `   • Adaptation: If metrics deviate from expected trajectory, ${sae.adaptationSuggestions[0]?.toLowerCase() ?? "adjust approach based on data"}. ${sae.learningPathway.feedbackLoops[0]?.toLowerCase() ?? "Review progress regularly"}.`,
        ``,
        `📊  CONFIDENCE ASSESSMENT`,
        `   • Confidence: ${Math.round(sve.confidenceScore * 100)}%`,
        `   • Reason: ${confidenceDisplay.reason}`,
        `   • Unknown factors: ${confidenceDisplay.unknownFactors.length > 0 ? confidenceDisplay.unknownFactors.join("; ") : "Limited data for specific domain — confidence will increase with real-world validation"}`,
        `   • Confidence floor: 45% | Confidence ceiling: 85% (per SPYRAL conservative estimation policy)`,
        ``,
        `📊  WHY THIS STRATEGY?`,
        ...
        `   • Iteration cadence: ${sae.learningPathway.iterationCadence}`,
        ``,
        `📊  CYCLE QUALITY`,
